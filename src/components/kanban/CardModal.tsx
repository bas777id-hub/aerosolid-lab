...
export default function CardModal({ cardId, onClose }: CardModalProps) {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    updateCard(cardId, {
      ...
      assigneeId,
      artifactLinks: {
        overleaf: overleaf || undefined,
        googleDrive: googleDrive || undefined,
      ...
      academicMetadata: {
        ...
        zoteroId: zoteroId || undefined,
        targetPublication: targetPublication || undefined,
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto p-4 shadow-2xl relative glow-accent animate-in fade-in duration-200">
        ...
        {/* Modal Status Header Bar */}
        <div className="flex items-center justify-between pb-3 border-b border-[var(--border-primary)] mb-4 gap-4">
          <div className="flex items-center gap-2">
            <span className="text-[10px] bg-[var(--accent-bg)] text-[var(--accent)] font-bold px-1.5 py-0.2 rounded font-mono uppercase">
              ...
              <div className="flex items-center gap-2">
                ...
                {isStaff && (
                  <button
                    ...
                    className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-[10px] px-2.5 py-1 rounded transition-all cursor-pointer shadow-sm"
                  >
                    Approve & Lock
                  </button>
                )}
        ...
        {!canEdit && (
          <div className="mb-4 bg-slate-500/10 border border-slate-500/20 text-[11px] text-[var(--text-secondary)] px-3 py-2 rounded flex items-center gap-2 leading-relaxed">
            ...
            <span>
              <strong>Read-Only Mode:</strong> This task has been submitted or finalized. Editing parameters requires PI or Postdoc administrative roles.
            </span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                ...
                <input 
                  ...
                  disabled={!canEdit}
                  required
                  className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1.5 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60"
                />
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  required
                  rows={3}
                  className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1.5 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60 leading-relaxed text-container-max"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    disabled={!canEdit}
                    className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1.5 text-xs text-[var(--text-primary)] focus:outline-none disabled:opacity-60 font-semibold"
                  >
                    {users.map(u => (
              ...
              {canEdit && (
                <div className="flex gap-2 w-full flex-wrap">
                  <button 
                    type="submit"
                    className="bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-white text-xs font-bold py-1.5 px-3 rounded flex items-center gap-1.5 transition-all cursor-pointer shadow-sm"
                  >
                  ...
                  {(card.columnId === 'inbox' || card.columnId === 'in_progress') && (
                    <button
                      type="button"
                      onClick={() => handleMoveToColumn('review')}
                      className="bg-indigo-950 border border-[var(--accent)] text-[var(--accent)] hover:bg-[var(--accent-bg)] text-xs font-bold py-1.5 px-3 rounded transition-all cursor-pointer shadow-sm"
                    >
                      Submit for Review
                    </button>
                  )}
                  {card.columnId === 'inbox' && (
                    <button
                      type="button"
                      onClick={() => handleMoveToColumn('in_progress')}
                      className="bg-[var(--bg-tertiary)] border border-[var(--border-primary)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] text-xs font-bold py-1.5 px-3 rounded transition-all cursor-pointer shadow-sm"
                    >
                      Start Work
                    </button>
                  )}
                  {card.columnId === 'in_progress' && (
                    <button
                      type="button"
                      onClick={() => handleMoveToColumn('inbox')}
                      className="bg-[var(--bg-tertiary)] border border-[var(--border-primary)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] text-xs font-bold py-1.5 px-3 rounded transition-all cursor-pointer shadow-sm"
                    >
                      Move to Backlog
                    </button>
                  ...
                  {card.columnId === 'review' && isStaff && (
                    <button
                      type="button"
                      onClick={() => handleMoveToColumn('in_progress')}
                      className="bg-amber-600/10 text-amber-600 border border-amber-600/25 hover:bg-amber-600/20 text-xs font-bold py-1.5 px-3 rounded transition-all cursor-pointer shadow-sm"
                    >
                  ...
                  <button 
                    ...
                    className="bg-red-650 hover:bg-red-500 text-white text-xs font-bold py-1.5 px-3 rounded flex items-center gap-1.5 transition-all cursor-pointer shadow-sm ml-auto"
                  >
            ...
            <div className="pt-3 border-t border-[var(--border-primary)]">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <h4 className="text-xs font-bold text-[var(--text-primary)] uppercase tracking-wide">
                  Feedback & Comments ({card.comments.length})
                </h4>
              </div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto mb-3 pr-1">
                {card.comments.length === 0 ? (
                  <div className="text-center py-4 text-[var(--text-tertiary)] text-[11px] italic">
                    No supervisor or student comments added.
                  </div>
                ) : (
                  card.comments.map(c => {
                      ...
                      <div key={c.id} className="bg-[var(--bg-secondary)] border border-[var(--border-primary)]/50 rounded p-2 text-xs space-y-1">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-1.5">
                            ...
                            {authorRoleBadge && (
                              <span className={`text-[7px] border px-1 rounded ${authorRoleBadge.bg}`}>
                                {authorRoleBadge.label.split(' ')[0]}
                              </span>
                            )}
                          </div>
                        ...
                        <p className="text-[11px] text-[var(--text-secondary)] leading-relaxed text-container-max">
                          {c.content}
                        </p>
                      </div>
                    );
              ...
              <form onSubmit={handleAddCommentSubmit} className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded px-2.5 py-1.5 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)]"
                />
                <button
                  type="submit"
                  className="bg-indigo-950 border border-[var(--accent)] text-[var(--accent)] hover:bg-[var(--accent-bg)] p-1.5 rounded transition-all cursor-pointer flex items-center justify-center shrink-0"
                  title="Send Comment"
                >
          ...
          {/* RIGHT COLUMN: Artifact Links & Academic Metadata */}
          <div className="space-y-4">
            
            {/* Native Artifact Links Card */}
              ...
              <div className="space-y-2.5">
                <div>
                  <div className="flex items-center justify-between mb-0.5">
                    <label className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase block">Overleaf LaTeX URL</label>
                    {overleaf && (
                      <a href={overleaf} target="_blank" rel="noopener noreferrer" className="text-[9px] text-[var(--accent)] hover:underline flex items-center gap-0.5">
                        Open Project <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    )}
                  </div>
                  <input 
                    ...
                    className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60 font-mono"
                  />
                </div>

                <div>
                  <div className="flex items-center justify-between mb-0.5">
                    <label className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase block">Google Drive / Dropbox</label>
                    {googleDrive && (
                      <a href={googleDrive} target="_blank" rel="noopener noreferrer" className="text-[9px] text-[var(--accent)] hover:underline flex items-center gap-0.5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-0.5">
                    <label className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase block">GitHub Repository</label>
                    {gitHub && (
                      <a href={gitHub} target="_blank" rel="noopener noreferrer" className="text-[9px] text-[var(--accent)] hover:underline flex items-center gap-0.5">
                        Open Repo <ExternalLink className="w-2.5 h-2.5" />
                      </a>
                    )}
                  </div>
                  <input 
            ...
            <div className="glass-panel p-3 rounded-lg border border-[var(--border-primary)] space-y-3">
              ...
              <div className="space-y-2.5">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-bold text-[var(--text-tertiary)] uppercase block mb-0.5">Digital Object Identifier (DOI)</label>
                    <input 
                      type="text" 
                      value={doi}
                      onChange={(e) => setDoi(e.target.value)}
                      disabled={!canEdit}
                      placeholder="e.g. 10.1021/acs.est..."
                      className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60 font-mono"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      disabled={!canEdit}
                      placeholder="e.g. DIANA2026_BIOCHAR"
                      className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60 font-mono"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2">
                    ...
                    <input 
                      ...
                      disabled={!canEdit}
                      placeholder="e.g. Chemosphere"
                      className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    disabled={!canEdit}
                    placeholder="@article{diana2026, author={Prince, D.}, journal={Chemosphere}, year={2026} ...}"
                    rows={3}
                    className="w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded p-1 text-[10px] text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent)] disabled:opacity-60 font-mono leading-normal"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-[var(--bg-secondary)] p-3 rounded-lg border border-[var(--border-primary)]/70 text-xs">
              <span className="text-[10px] font-bold text-[var(--text-primary)] block mb-1">
                Supervisory Approval Flow
              </span>
              <p className="text-[10.5px] text-[var(--text-secondary)] leading-relaxed">
                When a draft is ready, the assignee moves the card to the <strong className="text-[var(--text-primary)]">Submitted for Review</strong> column.
                The card is immediately locked. The student cannot edit or retrieve the card until approved. 
                Prof. Pendelton or Dr. Beatrice Vance must click the <strong className="text-[var(--text-primary)]">Approve & Lock</strong> button to finalize.
              </p>
            </div>
          </div>
